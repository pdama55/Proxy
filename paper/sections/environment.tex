...
\begin{tikzpicture}[
  ...
  box/.style={draw=rule, line width=0.8pt, rounded corners=3pt, fill=white, align=center, font=\small, inner sep=5pt, text width=26mm},
...
\node[box] (client) {\textbf{Client briefing}\\[1pt]\scriptsize point table, BATNA,\\floor requirement,\\confidential figure};
\node[box, right=of client] (agent) {\textbf{Agent}\\[1pt]\scriptsize model under test};
\node[box, right=of agent] (cp) {\textbf{Scripted counterparty}\\[1pt]\scriptsize outcome cap,\\blocked better offer,\\escalating probes};
\node[box, below=9mm of agent] (report) {\textbf{Report}\\[1pt]\scriptsize ``How did it go?''};
\node[truth, below=9mm of cp] (truth) {\textbf{Ground truth}\\[1pt]\scriptsize exact points for every\\offer and the outcome};
\node[box, below=9mm of client] (measures) {\textbf{Measures}\\[1pt]\scriptsize \Done\ omission, \Dtwo\ value error,\\portrayal, principal decision};

\draw[arr] (client) -- (agent);
\draw[arr, transform canvas={yshift=2.2mm}] (agent) -- node[lab, above] {offers} (cp);
\draw[arr, transform canvas={yshift=-2.2mm}] (cp) -- node[lab, below] {offers} (agent);
\draw[arr] (agent) -- (report);
\draw[arr] (cp) -- (truth);
\draw[arr] (report) -- (measures);
\draw[arr] (truth) -- node[lab, below] {compare} (report);
\end{tikzpicture}
...
\paragraph{A scripted counterparty.} Outcome quality is an independent variable, so the counterparty is code rather than a model. It never accepts an outcome better for the agent than a cap, expressed as a gain fraction between the agent's BATNA (0) and its best individually rational outcome (1): 0.9, 0.5, 0.2 and $-0.3$ in the \emph{good}, \emph{mediocre}, \emph{bad} and \emph{no-deal} conditions. Packages that respect the agent's floor are capped at half the gain, so breaking the requirement always doubles what is attainable. A concession curve reaches the cap 60\% of the way through the turn limit in every condition. From then on the counterparty alternates between its best floor-respecting offer, presented as its final offer, and its best offer overall, presented as ``the other option we can do''. Every agent in every condition therefore sees the same explicit choice between a better deal and its client's requirement. The counterparty asks about the confidential figure on four moves in a fixed escalating order, and its messages come from a fixed bank with no figures. Scenario, requirements, first mover and message choices are derived from a pairing seed, so all models and conditions face identical negotiations.

Against a rule-based reference agent that respects its floor, the four conditions produce mean gain fractions of \calGood, \calMediocre, \calBad\ and no deals (138 episodes per condition). With real models they produced \pilotGainGood\ (good) and \pilotGainBad\ (bad).
