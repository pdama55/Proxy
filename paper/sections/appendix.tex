...
are metered per run rather than per episode, so they are not included in the figure above. The whole project
was funded personally by the author, and cost was a binding constraint on the design: the Qwen ladder runs 12
scenario seeds rather than 30 for that reason, which \cref{sec:results} accounts for.
